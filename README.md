...
```bash
just           # 列出所有可用命令
just test      # 运行测试（uv run pytest）
just lint      # 代码检查（ruff check --fix）
just format    # 代码格式化（ruff format）
...
| 命令          | 等价于                     | 说明                                       |
| ------------- | -------------------------- | ------------------------------------------ |
| `just test`   | `uv run pytest`            | 运行所有测试                               |
| `just lint`   | `uv run ruff check . --fix` | 自动修复 lint 问题                         |
| `just format` | `uv run ruff format .`      | 格式化代码                                 |
...
- `basedpyright` - 类型检查
- `ruff` - 代码检查与格式化
- `nonebot-adapter-onebot` - OneBot V11 测试适配器
- `nonebot2[fastapi]` - NoneBot 测试运行环境
...
asyncio_default_fixture_loop_scope = "session" # NoneBug 初始化 fixture 使用 session loop
asyncio_default_test_loop_scope = "session"    # NoneBot 插件行为测试默认使用 session loop
```
...
uv run pytest tests/integration         # 仅运行集成测试
...
- codecov/codecov-action@v6   # 上传覆盖率（OIDC 认证）
```
...
│       ├── initialize.yml  # 模板初始化
│       └── template-sync.yml # 上游同步
├── src/
│   └── nonebot_plugin_template/  # 插件源码
│       ├── __init__.py           # 插件元数据与入口导入
│       ├── config.py             # 配置模型定义
│       └── handlers.py           # 初始命令与 plugin_config 创建
├── tests/                  # 测试代码
...
test = [
  ...
  "nonebot-adapter-onebot>=2.4.6,<3.0.0", # OneBot V11 测试适配器
  "nonebot2[fastapi]>=2.4.4,<3.0.0",   # NoneBot 测试运行环境
  "nonebug>=0.4.3,<1.0.0",             # NoneBot 测试框架
  "pytest-asyncio>=1.3.0,<1.4.0",      # 异步测试
